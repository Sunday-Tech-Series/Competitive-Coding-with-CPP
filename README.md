...
```
- _Image of Interactive console_
![image](images/basic-io/01.png?raw=true)
...
All the platforms for competitive coding usually use this method for input and output as it is comparatively much faster.

> _**Note:** We need to provide input before executing the code which was not in the case of interactive console._ 
...
```
- _Image of STDIN_
![image](images/basic-io/02.png?raw=true)

- _Image of STDOUT_
![image](images/basic-io/03.png?raw=true)
...
```
- _Image of STDERR_
![image](images/basic-io/04.png?raw=true)
...
![image](images/datatypes/DatatypesInC.png?raw=true)
...
![image](images/datatypes/ModifiersInC.png?raw=true)
...
![image](images/control-stmt/index.png?raw=true)
...
### Decision Statements
these statements decides the flow of program on the basis of a deciding condition.
...
## Arrays
An array is a collection of similar data items stored at contiguous memory locations and each elements can be accessed directly using indices of it. They can be used to store collection of primitive data types such as int, float, double, char, etc of any particular type. To add to it, an array in C/C++ can store derived and user-defined data types such as the structures, pointers etc. Given below is the picture representation of an array.

![image](images/datatypes/Array.png?raw=true)

- _Syntax to declare an array._
```cpp
int arr1[10];
...
int arr2[n];
```
- _Syntax to initialize an array._
```cpp
int arr[] = { 10, 20, 30, 40 }
...
- _A program to demonstrate Arrays._
```cpp
int main()
{
    int arr[5];
...
**Space complexity** is the total memory consumed by an algorithm to run.   
**Time complexity** is the amount of time taken by an algorithm to run, as a function of the length of the input. It measures the time taken to execute each statement of code in an algorithm. 

> **Note:** Now-a-days we require algorithms to be faster even if they consume a little bit more space. 
...
- **O(n<sup>2</sup>):** You go and ask the first person of the class, if he has the pen. Also, you ask this person about other 99 people in the classroom if they have that pen, now you repeat this same process to the other 99 people, 
This is what we call O(n<sup>2</sup>).  
- **O(n):** If you are going and asking each student individually it will be O(N).  
- **O(log n):** Now if you divide the whole class into two groups, then ask: “Is it on the left side, or the right side of the classroom?” Then you further divide the side into two and ask again, and so on. Repeat the process till you are left with one student who has your pen. This is what you mean by O(log n).  

The above time complexities we saw, divide and conquer will have a time complexity of O(log n), whereas the time complexity of bubble sort would be O(n<sup>2</sup>).

- _Example to show difference between O(n) & O(n<sup>2</sup>)._
```cpp
#include <iostream>
using namespace std;
...
{
    int count = 0;
    for(int i=0; i<50; i++){    //Order of n
        count++;
    }
    cout<<"Number of times the loop has run: "<<count;
    /*
    int count = 0;
    for(int i=0; i<50; i++){             //Order of n²
        for(int j=0; j<50; j++){
...
## Algorithms
An Algorithm is a step by step procedure to solve any given problem.
They are of many types, mainly

- **Greedy Algorithm:** If a Greedy Algorithm can solve a problem, then it generally becomes the best method to solve that problem as the Greedy algorithms are in general more efficient than other techniques but keep in mind Greedy algorithms cannot always be applied.
As the name suggests, they always makes the choice that seems to be the best at that moment hoping that it will provide the most optimized solution.

- **Divide and Conquer:** This technique is very common while using algotrithms, the concept is dividing the problem into subparts solving it and doing this recursively until the sub problem is solved.
Algorithms like Quick Sort, Merge Sort follow this approach.

- **Graph algorithms:** These algorithms can further be divided into two types Depth First & Breadth First. They are very efficient if we need to find the connectivity, or the shortest part between two points.
It is a data structure that consists of a finite set of nodes and edges, nodes being the points of interests and edges being the paths to traverse between them.
Graphs are used to represent many real-life applications: mainly networks. The networks may include paths in a city or telephone network or circuit network. Graphs are also used in social networks like linkedIn, Facebook. For example, in Facebook, each person is represented with a vertex(or node). Each node is a structure and contains information like person id, name, gender, and more.


- **Linear Search:** 
A simple approach is to do a linear search, _i.e._ 
...
- **Binary Search:** 
It is used to search a sorted array by repeatedly dividing the search interval in half. Begin with an interval covering the whole array. If the value of the search key is less than the item in the middle of the interval, narrow the interval to the lower half. Otherwise, narrow it to the upper half. Repeatedly check until the value is found or the interval is empty. The idea of binary search is to use the information that the array is sorted and reduce the time complexity to O(Log n). 

- **Bubble Sort:** 
It is a simple sorting algorithm. This sorting algorithm is comparison-based algorithm in which each pair of adjacent elements is compared and the elements are swapped if they are not in order. This algorithm is not suitable for large data sets as its average and worst case complexity are of Ο(n<sup>2</sup>) where n is the number of items.

![image](images/65.png?raw=true)

- _A Program to demonstrate Bubble Sort._
```cpp
#include<iostream>
using namespace std;
 
void swap(int *x, int *y){      //Function for swapping the elements
...
void bubbleSort(int a[], int n){ //Function implementing bubble sort
...
![image](images/67.png?raw=true)

- _A Program to Demonstrate Quick Sort._
```cpp
#include<iostream>
using namespace std;
...
void quickSort(int arr[], int low, int high){
    if (low < high){
        ...
        quickSort(arr, low, pi - 1);  //Sort elements before and after partition separately
...
int main(){
    ...
    int arr[] = {64, 34, 25, 12, 22, 11, 90, 7, 4, 91, 99, 100, 106, 765, 1, 0, 349};
...
```
> _**Note:** There are many other Searching and Sortig algorithms but here we discussed only the important ones._
...
![image](images/hackerrank-prof/01.png?raw=true) 
- _**Step 3:** Enter your personal details and Submit the form._
![image](images/hackerrank-prof/02.png?raw=true)
...
![image](images/hackerrank-prof/03.png?raw=true)
- _**Step 6:** Start solving problems here for practice._
![image](images/hackerrank-prof/04.png?raw=true)
...
![image](images/contest/01.png?raw=true)
...
We have another Webinar next sunday _i.e._ 20<sup>th</sup> June, 2021 on the topic _**"GUI Game Programming with Java"**_ the details will be shared with you all soon.  
<!-- You can register [here](#) for the same. --> 
![image](images/83.png?raw=true)
